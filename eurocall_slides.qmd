...
- Rhetorical moves are often **implicit** and discipline-specific.
- Traditional margin comments (*"needs better flow"*, *"show your contribution"*) can be opaque for students
...
* Zero technical barrier for teachers and students.
* Interactive exploration of reference corpora.
* Drag-and-drop custom text upload.
* Instant visual side-by-side comparisons.
:::

::: {.column width="50%"}
### Python Package (`movestepviz`)
* Open-source on PyPI (`pip install movestepviz`).
* Generates publication-ready DNA-strip diagrams.
* Pre-loaded with standard rhetorical schemes.
* Fully customizable colors, labels, and layouts.
:::
:::

---

## Anatomy of a Rhetorical DNA Strip

Each colored cell represents one functional sentence/clause:

```text
┌──────┐  ┌──────┐  ┌──────┐  ┌──────┐  ┌──────┐
│  1a  │  │  1b  │  │  1c  │  │  2b  │  │  3a  │
└──────┘  └──────┘  └──────┘  └──────┘  └──────┘
 Move 1    Move 1    Move 1    Move 2    Move 3
 Step 1    Step 2    Step 3    Step 1b   Step 1a
 (Territory)         (Lit)      (Gap)    (Purpose)
```

* **Color Families**: Each Move has a distinct color (Green = M1, Amber = M2, Blue = M3, Purple = M4, etc.).
* **Tonal Gradients**: Steps within the same move share shades of the family for intuitive hierarchy.
* **Readable In-Cell Labels**: Compact labels (e.g. `1a`, `1b`, `2a`, `3a`) maintain visual clarity without clutter.




## Built-In Genre & Section Schemes

MoveStepViz 0.2.2 comes pre-loaded with standard genre models:
...
| **`Yang_Allison2003_Results`** | **Results** Section | Yang & Allison (2003) |
| **`Yang_Allison2003_Discussion`** | **Discussion** Section | Yang & Allison (2003) |

| **Custom Schemes** | Any section / genre | Teacher-defined JSON or CSV |

---

# Web-based Graphical User Interface {background-color="#cbd5e1"}

## 1. More Intuitive Input to MoveStepViz {background-color="#d78aefff"}
:::: {.columns}
::: {.column width="60%"}
...
## 3. Student Self-Assessment & Peer Review {background-color="#d78aefff"}
